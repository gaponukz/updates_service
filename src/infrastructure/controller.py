...
from src.application.usecase import IVersionsUsecase
from src.infrastructure.authentication import admin_required
...
async def on_set_current_version(
    ...
    try:
        service.set_current_version(version)
...
async def on_get_build_info(
    ...
    try:
        return service.get_by_version(version)
...
async def on_get_current_version(service: IVersionsUsecase = Depends(), api_key: str = Depends(admin_required)) -> dto.AllVersionDto:
    return dto.AllVersionDto(
        current=service.get_current_version(),
        awiable=service.get_sorted_versions()
    )

@app.get("/versions/download")
async def root(current_user_version: entities.VersionSymbol, service: IVersionsUsecase = Depends()) -> FileResponse:
    current = service.get_current_version()
    ...
    if current_version.is_bigger_than(user_version):
        build = service.get_by_version(current)
